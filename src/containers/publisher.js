...
function intent({ DOM, HTTP, fractal, props }) {
    const title$ = DOM.select('form input#title')
        .events('input')
        .map(event => ({ field: 'title', value: event.target.value }));

    const content$ = DOM.select('form textarea#content')
        .events('input')
        .map(event => ({ field: 'content', value: event.target.value }));

    const category$ = DOM.select('form select#category')
        .events('change')
        .map(event => ({ field: 'category', value: event.target.value }));

    const step$ = DOM.select('.step .step-item')
        .events('click')
        .map(event => parseInt(event.currentTarget.dataset.step || 0));

    const checkboxes$ = DOM.select('form input[type="checkbox"]')
        .events('change')
        .map(event => ({
    ...
    const submit$ = DOM.select('form')
        .events('submit', { preventDefault: true })
        .map(event => event.currentTarget);

    const publish$ = DOM.select('a#publish')
        .events('click')
        .map(event => event.currentTarget)
        .remember();
    ...
    const routeState$ = props.router$.filter(
        action => action.page == 'publish'
    );

    const form$ = xs.merge(title$, content$, category$, checkboxes$);
...
function model(actions) {
    const reducers$ = xs.merge(
        ...
        // Route state
        actions.routeState$.map(action => ({ publisher }) => ({
            ...publisher,
            ...action.location.state,
        })),

        // Map form changes to reducers.
        actions.form$.map(f => ({ publisher }) => ({
            ...publisher,
            [f.field]: f.value,
        })),

        // Next step submits
        actions.submit$.map(() => ({ publisher }) => ({
            ...publisher,
            step: publisher.step <= 1 ? publisher.step + 1 : 2,
        })),

        // Manual step change
        actions.step$.map(step => ({ publisher }) => ({
            ...publisher,
            step: publisher.step >= step ? step : publisher.step,
        })),

        // Loading state (block actions)
        actions.publish$.map(() => ({ publisher }) => ({
            ...publisher,
            saving: true,
        }))
    );
    ...
    const history$ = xs.merge(
        ...
        actions.submit$.compose(sampleCombine(actions.state$)).map(params => ({
            type: 'push',
            pathname: '/publicar',
            state: params[1],
        })),
        // Once published go to the post page.
        actions.published$.map(({ post }) => ({
            type: 'push',
...
function postInfo({ categories, publisher }) {
    const { title, isQuestion, disabledComments, pinned, category } = publisher;
                        ...
                        categories.map(c =>
                            optgroup(
                                ...
                                c.subcategories.map(s =>
                                    option({ attrs: { value: s.id } }, s.name)
                                )
                            )
                        )
                    )
...
function postReview(state) {
    ...
    return div([
        ...
        form('.pv3.mt3', [
            ...
            hr(),
            virtualize(
                `<div class="post-preview pb2">${md.render(content)}</div>`
            ),
            a(
