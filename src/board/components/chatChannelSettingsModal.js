...
export const ChatChannelSettings = withRouter(
    function ChatChannelSettings(props) {
        const { channel, effects, isOpen, onRequestClose, ...otherProps } =
            props;
        ...
        async function onSubmit(event) {
            ...
            const state = await effects.updateChatChannelConfig(
                channel,
                updated
            );
            props.history.push(
                deleteChannel
                    ? `/chat/${state.site.chat[0].name}`
                    : `/chat/${name}`
            );
            onRequestClose();
        }
