...
import { h, div, section, i, p, button, a, span, ul, li, article, h3, h1, footer, small, select, option, form, label, input, optgroup, textarea } from '@cycle/dom'
    ...
    return form('.pv3', { attrs: { id: 'update-post' } }, [
        div('.form-group.pb2', [
            ...
            select('.form-select', { attrs: { id: 'category', required: true } },
                [
                    option('Selecciona una categoría para la publicación')
                ].concat(categories.map(c =>
                    optgroup({ attrs: { label: c.name } },
                        c.subcategories.map(s => option({ attrs: { value: s.id }, props: { selected: s.id === post.category } }, s.name))
                    )
                ))
            )
        ]),
        div('.form-group.pb2', [
            ...
            input('#title.form-input', { attrs: { type: 'text', value: post.title, placeholder: 'Escribe el titulo de tu publicación o pregunta...', required: true } })
        ]),
        div('.form-group.pb2', [
        ...
        div('.form-group.pb2', [
            div('.form-group',
                label('.form-switch.normal', [
                    input({ attrs: { type: 'checkbox', name: 'isQuestion', checked: false } }),
                    i('.form-icon'),
            ...
            div('.form-group',
                label('.form-switch.normal', [
                    input({ attrs: { type: 'checkbox', name: 'disabledComments', checked: false } }),
                    i('.form-icon'),
            ...
            div('.form-group',
                label('.form-switch.normal', [
                    input({ attrs: { type: 'checkbox', name: 'pinned', checked: false } }),
                    i('.form-icon'),
                    'Publicar como importante'
                ]),
            )
        ]),
        input('.btn.btn-primary.btn-block', { attrs: { type: 'submit', value: 'Guardar publicación' } })
    ])
}
