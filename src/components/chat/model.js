...
export function model(actions) {
    ...
    const configR$ = actions.config$.map(config => state => ({
        ...state,
        config: {
            ...state.config,
            ...config,
    ...
    const highlightR$ = actions.rhighlighted$.map(item => state => ({
        ...state,
        highlighted: item,
    }));
    const messageR$ = actions.msg$.map(message => state => ({
        ...state,
    ...
    const messagesR$ = xs
        ...
        .map(packed => state => {
            ...
            return {
                ...
                missing:
                    state.lock === false
                        ? state.missing + packed.list.length
                        : state.missing,
            };
        });
