...
export function model(actions) {
     ...
     */
    const storage$ = actions.logoutLink$.map(() => ({
        key: 'id_token',
        action: 'removeItem',
    }));
     ...
     */
    const openNotificationsR$ = requestNotifications$.map(() => state => ({
        ...state,
        resolving: {
            ...state.resolving,
            notifications: true,
    ...
    const notificationsR$ = actions.notifications$.map(
        notifications => state => ({
            ...state,
            resolving: {
                ...state.resolving,
    ...
    const incomingNotificationR$ = actions.userChan$
        .filter(ev => ev.fire == 'notification')
        .map(ev => state => ({
            ...state,
            resolving: {
                ...state.resolving,
    ...
    const reducers$ = actions.modalLink$.map(action => state => ({
        ...state,
        modal: {
            ...state.modal,
            active: true,
    ...
    const beep$ = actions.userChan$.filter(
        ev => ev.fire == 'notification' && ev.count > 0
    );
