 ...
 * @param history
 */
export function intent({ DOM, HTTP, props, glue }) {
    /**
    ...
    const fetch$ = xs
        .merge(
            ...
            props.router$
                .filter(
                    action =>
                        action.page == 'board' || action.page == 'category'
                )
                .map(action => ({
                    type: 'reload',
                    category:
                        'category' in action && action.category !== false
            ...
            props.router$
                ...
                .filter(
                    action =>
                        'reloadPosts' in action.location.state &&
                        action.location.state.reloadPosts === true
                )
                .map(() => ({
                    type: 'bootstrap',
                })),
