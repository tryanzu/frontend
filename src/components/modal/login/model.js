...
export function model(actions) {
    ...
    const requestPassword$ = actions.sent$
        .filter(name => name == 'forgot')
        .compose(sampleCombine(actions.fields$))
        .map(params => {
            const [email] = params;
            return {
     ...
     */
    const fieldsR$ = actions.fields$.map(([email, password]) => state => ({
        ...state,
        email,
        password,
    }));
    ...
    const forgotR$ = actions.forgot$.map(show => state => ({
        ...state,
        showForgotPassword: show,
    }));
    const rememberMeR$ = actions.rememberMe$.map(active => state => ({
        ...state,
        rememberMe: active,
    }));
    ...
    const recoverR$ = actions.recover$.map(() => state => ({
        ...state,
        sentRecover: true,
    }));

    const state$ = xs
        .merge(fieldsR$, sentR$, tokenR$, forgotR$, rememberMeR$, recoverR$)
