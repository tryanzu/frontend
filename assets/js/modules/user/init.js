...
UserModule.factory('User', ['$resource', function($resource) {
    return $resource(layer_path + 'users/:user_id', {
        user_id: '@user_id'
    });
}]);

// User Profile controller
UserModule.controller('UserController', ['$scope', 'User', '$routeParams', 'Feed', 'Upload', '$http', '$timeout', '$location',
    function($scope, User, $routeParams, Feed, Upload, $http, $timeout, $location) {
        ...
        $scope.updatePassword = function() {
            ...
            $http.put(layer_path + "user/my", {
                password: $scope.password_update.password
            }).then(function success(response) {
                console.log(response);
                $scope.password_update.show_message = true;
        ...
        $scope.saveUsername = function() {
            if ($scope.user.info.name_changes < 1) {
                ...
                $http.put(layer_path + "user/my", {
                    username: $scope.new_data.username
                }).
...
UserModule.controller('UserRecoveryController', ['$scope', '$http', '$routeParams',
    function($scope, $http, $routeParams) {
        ...
        $scope.updatePassword = function() {
            ...
            $http.put(layer_path + "auth/recovery-token/" + $routeParams.token, {
                password: $scope.password_update.password
            }).then(function success(response) {
                console.log(response);
                $scope.password_update.show_message = true;
